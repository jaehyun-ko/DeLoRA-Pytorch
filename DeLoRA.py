...
class DeLoRALinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, r: int = 4, lambda_init: float = 1e-3, bias: bool = True):
        ...
        # Use Kaiming initialization for initial diversity.
    ...
    def reset_parameters(self):
        ...
        # Optionally zero-init the low-rank updates so that the module starts as an identity.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Note: torch.norm(self.weight, p='fro') computes the Frobenius norm of the base weight.
        update = (self.lambda_ * self.weight.norm(p='fro') / self.r) * (B_norm @ A_norm)
        # Combine with the frozen base weight.
        W_adapted = self.weight + update
        # Linear transformation using the adapted weight.
        return F.linear(x, W_adapted, self.bias)

    def merge_weights(self):
        """
        Merges the low-rank update into the base weight.
        After merging, the adapter update can be removed for efficient inference.
        """
        eps = 1e-6
        B_norm = self.B / (self.B.norm(dim=0, keepdim=True) + eps)
        A_norm = self.A / (self.A.norm(dim=1, keepdim=True) + eps)
        update = (self.lambda_ * self.weight.norm(p='fro') / self.r) * (B_norm @ A_norm)
        self.weight.data += update.data
        # Optionally, zero out the adapter parameters.
        nn.init.zeros_(self.A)
        nn.init.zeros_(self.B)

    def unmerge_weights(self):
        """
        Optionally revert the merge operation (requires saving original weight).
        """
        pass
...
if __name__ == '__main__':
    ...
    y = de_lora(x)
    print(y.shape)  # Expected output: [8, 64]
